...
<c:set var="pathurl" value="<%=request.getContextPath() %>"></c:set>
<h4>${pathurl }</h4>
...
<c:if test="${param.age>=20 }">
	<h2 style="color: blue">당신은 성인</h2>
</c:if>
<c:if test="${param.age<20 }">
	<h2 style="color: red">당신은 미성년자</h2>
</c:if>
<hr>
<c:choose>
...
<c:choose>
	<c:when test="${param.nara=='캐나다' }">
		<h3 style="color:green;">캐나다의 메이플스토리는 아름답습니다</h3>
	</c:when>
</c:choose>
<c:choose>
	<c:when test="${param.nara=='미국' }">
		<h3 style="color:orange;">미국의 피자는 아름답습니다</h3>
	</c:when>
	<c:otherwise>
		<h3 style="color: gray;">${param.nara } 는 어떤 나란가요?</h3>
	</c:otherwise>
</c:choose>
<hr>
<c:set var="str" value="red,green,blue,gray,pink,magenta"></c:set>
