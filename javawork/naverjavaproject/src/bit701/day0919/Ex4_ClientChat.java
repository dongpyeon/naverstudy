...
public class Ex4_ClientChat extends JFrame implements Runnable,ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		//메세지 입력 후 엔터를 누르거나 전송 버튼을 누를 경우 같은 코드가 실행 된다
	...
	public void run() {
		...
		//서버에 접속
		try {
			clinetSocket=new Socket(tfServerIp.getText(), 6000);
			//대화를 주고받을수 있게 io 클래스 얻기
			InputStream is=null;
			try {
				is=clinetSocket.getInputStream();
				br=new BufferedReader(new InputStreamReader(is));
				pw=new PrintWriter(clinetSocket.getOutputStream());				
			} catch (IOException e) {
...
//				e.printStackTrace();
			...
			//서버가 언제 메세지를 보낼 지 모르므로 while 문을 열어두고
	...
	private void setDesign() {
		...
		tfNickName=new JTextField("동편",10); //자기이름으로
		tfServerIp=new JTextField("192.168.0.30");
