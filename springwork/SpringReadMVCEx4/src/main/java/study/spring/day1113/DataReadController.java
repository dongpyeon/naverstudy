...
public class DataReadController {
	...
	//@RequestParam 자체가 사실 생략이 가능하다 (하지만 다른 어노테이션으로 잘못 인식하는 경우를 방지하기 위해 생략 안하는 게 좋긴 함
...
//	{
//		//jsp 처럼 읽어보자(스프링이에서는 이렇게 읽지는 않음
...
//	}
	...
	public String pro2(Model model,@RequestParam String message,@RequestParam String animal
			,@RequestParam(defaultValue = "이름이 입력되지 않았어") String name)
			/* null 값이 들어오더라고 에러가 안나게 할 경우 (에러코드 400) required =false
			 * null 일 경우 기본값을 설정하고 싶을때 defaultValue 설정*/
	{
	...
	public String pro3(@ModelAttribute ShopDto dto,@RequestParam String today, Model model) //model 에 shopDto 라는 이름으로 저장
	//public String pro3(@ModelAttribute("dto") ShopDto dto) //model 에 dto 라는 이름으로 저장
	{
	...
	public String process4(Model model,@RequestParam Map<String, String> map)// 폼태그의 name이 키로 입력값은 value로 저장
	{
